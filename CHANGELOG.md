...
### Fixed
- **`vouch compile` ignores archived pages in TAKEN TOPICS / collisions** (#700):
  `build_prompt` and `compile_kb` treated every on-disk page as taken, so
  archiving a bad topic permanently blocked redraft under the same title.
  Both now use the same live set as recall / wiki (exclude `ARCHIVED`).
